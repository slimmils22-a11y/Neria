...
S.push(
  ...
  p("Le seul appel réseau externe automatique et obligatoire est celui effectué par Neria vers son propre serveur de licences (neriasoftware.com), une fois toutes les 24h maximum, pour vérifier la validité de la clé. Données transmises : clé de licence, nom de domaine, version du module. Aucune donnée personnelle client n'est transmise. En cas d'indisponibilité du serveur, la dernière vérification en cache reste valide indéfiniment — les envois d'emails ne sont jamais interrompus par une panne côté éditeur."),
  p("Les autres connexions externes décrites dans ce document (DeepL, Gmail Postmaster Tools, Google Search Console, PageSpeed Insights, Semrush/Moz, webhooks sortants, boîte IMAP pour les bounces) sont toutes optionnelles : elles ne s'activent que si vous configurez explicitement une clé API ou des identifiants pour l'intégration concernée."),
  new Paragraph({ children: [new PageBreak()] })
);
