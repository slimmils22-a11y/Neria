<?php
/**
 * Régression : campagnes saisonnières ('grandparents_day', 'black_friday', 'new_year', 'early_access', 'private_sale') ne doivent laisser
...
function run_test(): array
{
    ...
    try {
        ...
        return ['pass' => true, 'message' => 'grandparents_day, black_friday, new_year, early_access, private_sale ne laissent aucune variable résiduelle avec un jeu de variables réaliste'];
    } finally {
        $db->execute(
            "DELETE FROM {$prefix}neria_log
             WHERE class = 'EmailRenderer' AND date_add >= '{$startedAt}'
               AND template IN ('grandparents_day', 'black_friday', 'new_year', 'early_access', 'private_sale')"
        );
    }
}
