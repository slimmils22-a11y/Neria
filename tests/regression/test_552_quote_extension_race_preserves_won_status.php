 ...
 *
 * C'est un des UPDATE de cette méthode à modifier `status` — donc exposé
 * à une vraie fenêtre de course (TOCTOU) : entre le SELECT
 ...
 * cron écrasait inconditionnellement ce `'won'` fraîchement posé, en plus
 * d'avoir déjà envoyé au client un email « offre de prolongation » pour un
 * devis qu'il venait tout juste d'accepter.
 *
 ...
 * le nouveau statut.
 *
 * Mis à jour le 09/09/2026 (round 329, hors round) : le chemin nominal de
 * cet UPDATE ne passe plus à 'expired' mais à 'extended' (avec
 * expiry_date réellement repoussée de 7 jours) — voir
 * project_neria_quote_extension_promise_not_honored.md / test_662 pour le
 * détail du correctif. La protection anti-course elle-même (AND
 * status = 'active') est inchangée et reste vérifiée ici à l'identique.
 *
 ...
 * l'UPDATE doit s'appliquer normalement et passer à 'extended').
 */
...
function run_test(): array
{
    ...
    $posExt = strpos($src, "SET sent_extension = 1, status = \\'extended\\', expiry_date = DATE_ADD(expiry_date, INTERVAL 7 DAY), date_upd = NOW()");
    neria_assert($posExt !== false, "L'UPDATE de la section 3 (offre de prolongation) est introuvable — jeu de test invalide");
    $updateBody = substr($src, $posExt, 260);
    neria_assert(
    ...
    try {
        ...
        neria_assert(
            $statusAfterWon === 'won',
            "L'UPDATE de section 3 écrase encore le statut 'won' — régression du bug corrigé le 04/09/2026 (round 296) : statut obtenu '{$statusAfterWon}' au lieu de 'won' préservé"
        );
        ...
        neria_assert(
            $statusAfterActive === 'extended',
            "jeu de test invalide ou régression : l'UPDATE ne passe plus un devis réellement 'active' à 'extended' (obtenu '{$statusAfterActive}')"
        );

        return [
            'pass'    => true,
            'message' => "sendQuoteExpiryReminders() (section 3, offre de prolongation) préserve désormais un statut 'won' posé entre-temps par le client, tout en continuant de prolonger normalement un devis resté 'active' — bug corrigé le 04/09/2026 (round 296)",
        ];
    } finally {
