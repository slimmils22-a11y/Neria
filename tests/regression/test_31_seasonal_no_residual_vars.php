<?php
/**
 * Régression : campagnes saisonnières ('diwali', 'hanukkah', 'lunar_new_year', 'mothers_day', 'fathers_day') ne doivent laisser
...
function run_test(): array
{
    ...
    try {
        ...
        return ['pass' => true, 'message' => 'diwali, hanukkah, lunar_new_year, mothers_day, fathers_day ne laissent aucune variable résiduelle avec un jeu de variables réaliste'];
    } finally {
        $db->execute(
            "DELETE FROM {$prefix}neria_log
             WHERE class = 'EmailRenderer' AND date_add >= '{$startedAt}'
               AND template IN ('diwali', 'hanukkah', 'lunar_new_year', 'mothers_day', 'fathers_day')"
        );
    }
}
