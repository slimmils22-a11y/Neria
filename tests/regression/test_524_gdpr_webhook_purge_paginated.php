 ...
 * lots de 2000 lignes (`LIMIT`/`OFFSET`), le comportement fonctionnel
 * (purge exacte, pas de sous-chaîne — round 144) restant strictement
 * inchangé.
 *
...
function run_test(): array
{
    ...
    try {
        ...
        neria_assert(
            strpos($body, 'ORDER BY `id_webhook` ASC LIMIT {$whChunkSize} OFFSET {$whOffset}') !== false
            && strpos($body, 'while ($rowCount === $whChunkSize);') !== false,
            "GdprAuditManager::purgeCustomerData() n'a plus la boucle LIMIT/OFFSET attendue pour neria_webhook_queue — régression du bug corrigé le 01/09/2026 (round 275)"
        );
