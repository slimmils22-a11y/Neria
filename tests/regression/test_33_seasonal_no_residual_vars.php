<?php
/**
 * Régression : campagnes saisonnières ('exclusive_preview', 'gift_ideas') ne doivent laisser
...
function run_test(): array
{
    ...
    try {
        ...
        return ['pass' => true, 'message' => 'exclusive_preview, gift_ideas ne laissent aucune variable résiduelle avec un jeu de variables réaliste'];
    } finally {
        $db->execute(
            "DELETE FROM {$prefix}neria_log
             WHERE class = 'EmailRenderer' AND date_add >= '{$startedAt}'
               AND template IN ('exclusive_preview', 'gift_ideas')"
        );
    }
}
