...
Généré le 2026-09-20 10:37:56 — module v1.0.48 — **828 lignes** dans `inventory/matrix.csv`.
...
- Actions BO sans gabarit qui les déclenche (1) : search_customers
- Actions BO sans bannière ni réponse AJAX (6) : deliverability_score, health_pixel_test, preview, run_bounce_check, search_customers, send_test
...
- Dictionnaire admin : {"keys":3377,"missing_by_lang":[]}
...
169 méthodes publiques sur 606 ne sont citées dans aucun test (indicateur de lacunes, pas une preuve de bug). Détail par classe dans `inventory.json` › `method_coverage`.
