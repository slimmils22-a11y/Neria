...
CREATE TABLE IF NOT EXISTS `PREFIX_neria_certificate` (
    ...
    `product_name`    VARCHAR(255) NOT NULL DEFAULT '',
    `artisan_name`    VARCHAR(255) DEFAULT NULL COMMENT 'Round 196 : ajouté ici pour que les installs fraîches convergent avec upgrade-1.0.42.php — page de traçabilité publique (QR code du certificat)',
...
CREATE TABLE IF NOT EXISTS `PREFIX_neria_queue` (
    ...
    -- même panier abandonné) ne doit être mis en file qu'une fois PAR
    -- BOUTIQUE — id_shop inclus (même motif que neria_behavioral_sent) :
