...
class CertificateManager
{
    ...
    public function issue(
        ...
        //
        // Round 177 : le retry avec offset réduisait la probabilité de
        // collision sans l'éliminer — une fenêtre TOCTOU subsistait entre
        // serialExists() (lecture) et l'INSERT plus bas (écriture) : deux
        // émissions passant serialExists() au même instant pour le même
        // numéro se heurtaient ensuite toutes deux à la contrainte UNIQUE en
        // DB (pas de doublon réel), mais l'une des deux échouait visiblement
        // pour un certificat pourtant légitime. Un verrou nommé MySQL
        // (GET_LOCK, même pattern que TranslationHistoryManager) sérialise
        // désormais toute la résolution+réservation du numéro de série pour
        // fermer cette fenêtre — released dans le finally plus bas, après
        // l'INSERT.
        // Round 212 : $use_cache=false, même famille de bug que les rounds
        // 210-211 (cache SQL PrestaShop neutralisant GET_LOCK).
        $serialLockName = 'neria_certificate_serial';
        $serialLockAcquired = ((int) $this->db->getValue(
            "SELECT GET_LOCK('" . pSQL($serialLockName) . "', 5)",
            false
        )) === 1;
        if (!$serialLockAcquired) {
        ...
        try {
            ...
            if ($autoGenerated) {
                $serialNumber = $this->generateSerial(0, (int) $order->id_shop);
            }
            ...
            if ($this->serialExists($serialNumber)) {
                ...
                for ($offset = 1; $offset <= 5; $offset++) {
                    $candidate = $this->generateSerial($offset, (int) $order->id_shop);
            ...
            if (!$inserted) {
                ...
                // Round 212 : generatePdf() a déjà écrit le fichier PDF sur
                // disque AVANT cet INSERT — sans ce nettoyage, un échec
                // d'insertion laissait un fichier orphelin référencé nulle
                // part en DB (fuite de fichier silencieuse, incohérence
                // disque/DB persistante).
                $orphanPath = _PS_MODULE_DIR_ . 'neria/' . $pdfPath;
            ...
            $this->db->execute("SELECT RELEASE_LOCK('" . pSQL($serialLockName) . "')");
        }

        // ── Envoi email ───────────────────────────────────────────
        if ($sendEmail) {
            // Round 177 : (int) $customer->id_lang (langue brute du compte)
            // au lieu de $idLangProduct (langue RÉELLEMENT résolue par
            // resolveCertificateLang(), utilisée pour le PDF ci-dessus) —
            // contredisait directement la garantie de parité PDF/email
            // documentée en tête de cette méthode. Quand la résolution
            // diverge du compte client (ex. compte en 'en' mais adresse de
            // facturation FR avec auto-détection de langue active), le PDF
            // joint et le corps de l'email se retrouvaient dans deux
            // langues différentes.
            $err = $this->sendCertificateEmail(
                $customerEmail, $customerName, $productName,
                $serialNumber, $pdfContent, $order, $idLangProduct
            );
            if ($err !== '') {
    ...
    private function generatePdf(
        ...
        // Round 212 : $idShop transmis explicitement — même correctif que
        // $shopName ci-dessus (round 106), jamais étendu à ces 3 champs.
        // Sans lui, un employé en contexte "Boutique B" émettant un
        // certificat pour une commande de la "Boutique A" (chaque
        ...
        // Round 212 : $idShop transmis explicitement, même correctif que
        // $title/$subtitle/$bodyText ci-dessus.
        $qrEnabled = (bool) \Configuration::get(self::CFG_QR_ENABLED, null, null, (int) $order->id_shop);
        ...
        $qrBaseUrl = (string) \Configuration::get(self::CFG_QR_URL, null, null, (int) $order->id_shop);
        if ($qrBaseUrl === '') {
    ...
    private function generateSerial(int $offset = 0, ?int $idShop = null): string
    {
        // Round 212 : $idShop transmis explicitement pour CFG_SERIAL_PREFIX
        // — la numérotation elle-même reste volontairement GLOBALE (voir
        // commentaire ci-dessous), mais le PRÉFIXE est un réglage
        // personnalisable par boutique ; sans ce paramètre, un employé en
        // contexte "Boutique B" émettant pour une commande de la
    ...
    {
        // Round 212 : $use_cache=false — check-then-act d'unicité appairé
        // au GET_LOCK ci-dessus, même famille de bug que les rounds
        // 210-211.
