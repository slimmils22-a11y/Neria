...
try {
    $stmt = $pdo->prepare(
        "SELECT `id_shop`, `level`, `template`, `class`, `message`, `date_add`
...
// Round 15/09/2026 (bloc d, arbitrage produit) : ces 3 clés sont écrites
// via Configuration::updateValue() SANS $idShop explicite
// (HealthCheckManager::runAutoChecksIfDue()/runFullDiagnostic()) — le
// cœur PrestaShop les scope alors automatiquement par la boutique
// ambiante (Shop::getContextShopID(true)) dès que le multiboutique est
// actif. Un simple `LIMIT 1` sans tri ni id_shop pouvait donc afficher,
// sur cette page de secours SANS notion de "boutique en cours", les
// résultats d'une boutique arbitraire — potentiellement pas celle
// réellement en panne, au pire moment pour se fier à un mauvais chiffre.
// Corrigé en agrégeant : chaque boutique active est listée séparément
// avec SES PROPRES résultats, plutôt qu'une seule valeur ambiguë.
$shopsForHealth = [[ 'id_shop' => null, 'name' => '' ]];
try {
    $stmt = $pdo->prepare("SELECT `id_shop`, `name` FROM `{$prefix}shop` WHERE `active` = 1 ORDER BY `id_shop`");
    $stmt->execute();
    $realShops = $stmt->fetchAll();
    if ($realShops) {
        $shopsForHealth = $realShops;
    }
} catch (Exception $e) {
    // Table shop absente/illisible (install cassée au-delà de ce que
    // cette page peut diagnostiquer) — repli sur une entrée unique
    // (id_shop=null → lit la ligne globale, comportement historique).
}
$multiShopEmergency = count($shopsForHealth) > 1;

// Pour chaque boutique : résultats de santé, dernière exécution,
// échecs consécutifs — en préférant la ligne scopée à CETTE boutique
// (ORDER BY id_shop = X DESC) et en repliant sur la ligne globale
// (id_shop IS NULL) si l'install n'a pas (encore) le multiboutique actif.
$healthByShop = [];
$stmtHealth = $pdo->prepare(
    "SELECT `value`, `id_shop` FROM `{$prefix}configuration`
     WHERE `name` = ? AND (`id_shop` = ? OR `id_shop` IS NULL)
     ORDER BY (`id_shop` = ?) DESC LIMIT 1"
);
foreach ($shopsForHealth as $s) {
    $idShop = $s['id_shop'];
    $entry = ['name' => $s['name'] ?? '', 'results' => [], 'lastRun' => '', 'failures' => 0];
    try {
        $stmtHealth->execute(['NERIA_HEALTH_RESULTS', $idShop, $idShop]);
        $row = $stmtHealth->fetch();
        $entry['results'] = $row ? (json_decode((string) $row['value'], true) ?: []) : [];

        $stmtHealth->execute(['NERIA_HEALTH_LAST_RUN', $idShop, $idShop]);
        $row = $stmtHealth->fetch();
        $entry['lastRun'] = $row ? (string) $row['value'] : '';

        $stmtHealth->execute(['NERIA_CONSECUTIVE_FAILURES', $idShop, $idShop]);
        $row = $stmtHealth->fetch();
        $entry['failures'] = $row ? (int) $row['value'] : 0;
    } catch (Exception $e) {
        // Ligne conservée à ses valeurs par défaut ci-dessus.
    }
    $healthByShop[] = $entry;
}

// Agrégats globaux pour les KPIs d'ensemble en tête de page (somme/max
// toutes boutiques confondues) — le détail par boutique reste disponible
// plus bas.
$healthResults = $healthByShop[0]['results'] ?? [];
$healthLastRun = $healthByShop[0]['lastRun'] ?? '';
$consecutiveFailures = 0;
foreach ($healthByShop as $h) {
    $consecutiveFailures = max($consecutiveFailures, $h['failures']);
}
...
<div class="container">
  ...
  <!-- Contrôles de santé — une section PAR boutique active (round 15/09/2026,
       bloc d) : cette page n'a aucune notion de "boutique en cours", afficher
       un seul résultat arbitraire pouvait masquer la vraie boutique en panne. -->
  <?php foreach ($healthByShop as $h): if (empty($h['results'])) continue; ?>
  <div class="section">
    <div class="section__head">
      <span class="section__title">
        <?= htmlspecialchars(e18n($T, 'section_health_checks')) ?>
        <?php if ($multiShopEmergency && $h['name'] !== ''): ?>
          — <?= htmlspecialchars($h['name']) ?>
        <?php endif; ?>
      </span>
      <?php if ($h['lastRun']): ?>
        <span style="font-size:11px;color:#aaa;margin-left:auto;"><?= htmlspecialchars(e18n($T, 'last_diagnostic', $h['lastRun'])) ?></span>
      <?php endif; ?>
    </div>
    <div class="section__body">
      <div class="health-grid">
        <?php foreach ($h['results'] as $key => $result):
        ...
        <table id="logTable">
          <thead>
            <tr>
              ...
              <?php if ($multiShopEmergency): ?>
                <th><?= htmlspecialchars(e18n($T, 'th_shop')) ?></th>
              <?php endif; ?>
          ...
          <tbody>
          <?php
          // Round 15/09/2026 (bloc d) : neria_log possède bien sa propre
          // colonne id_shop (schéma confirmé round 359) — affichée
          // uniquement sur une install multiboutique, pour identifier
          // quelle boutique chaque entrée concerne réellement.
          $shopNamesById = [];
          foreach ($shopsForHealth as $s) {
              if ($s['id_shop'] !== null) {
                  $shopNamesById[(int) $s['id_shop']] = $s['name'];
              }
          }
          foreach ($logs as $log):
            $lv      = $log['level'] ?? 'info';
            ...
            $logShop = $shopNamesById[(int) ($log['id_shop'] ?? 0)] ?? ('#' . (int) ($log['id_shop'] ?? 0));
          ?>
            <tr data-level="<?= htmlspecialchars($lv) ?>"
              ...
              <?php if ($multiShopEmergency): ?>
                <td style="white-space:nowrap;"><?= htmlspecialchars($logShop) ?></td>
              <?php endif; ?>
